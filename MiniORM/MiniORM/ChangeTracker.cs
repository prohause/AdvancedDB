...
namespace MiniORM
{
        ...
        where T : class, new()
    {
        ...
        public IEnumerable<T> GetModifiedEntities(DbSet<T> dbSet)
        {
            ...
            foreach (var proxyEntity in AllEntities)
            {
                ...
                var entity = dbSet.Entities.Single(e =>
                    GetPrimaryKeysValues(primaryKeys, e).SequenceEqual(primaryKeyValues));
        ...
        {
            var monitorProperties = typeof(T).GetProperties().Where(pi => DbContext.AllowedSqlTypes.Contains(pi.PropertyType));

            var modifiedProperties = monitorProperties
                .Where(pi => !Equals(pi.GetValue(entity), pi.GetValue(proxyEntity))).ToArray();

            var ismModified = modifiedProperties.Any();
