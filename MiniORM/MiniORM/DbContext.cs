...
namespace MiniORM
{
    public abstract class DbContext
    {
            ...
            where TDbSet : class, new() where TCollection : class, new()
        {
            ...
            var isManyToMany = primaryKeys.Length > 1;
            ...
            where TEntity : class, new()
        {
            ...
            foreach (var foreignKey in foreignKeys)
            {
                ...
                foreach (var entity in dbSet)
                {
                    ...
                    var navigationPropertyValue = ((IEnumerable<object>)navigationDbSet).First(
                        currentNavigationProperty => navigationPrimaryKey.GetValue(currentNavigationProperty)
        ...
        private string GetTableName(Type tableType)
        {
            var tableName = ((TableAttribute)Attribute.GetCustomAttribute(tableType, typeof(TableAttribute)))?.Name ??
                            dbSetProperties[tableType].Name;

            return tableName;
        }

        private string[] GetEntitiesColumnNames(Type table)
        {
            var tableName = GetTableName(table);
